...
function fetchUrl(url: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(
      ...
      {
        headers: {
          "User-Agent": "Mozilla/5.0 (compatible; BendSurfWave/2.0)",
          Accept: "*/*",
        },
        timeout: 10000,
      },
...
// API endpoint to get all surf report data
app.get("/api/surf-data", async (req, res) => {
  try {
    ...
    if (openMeteoWeatherJson.status === "fulfilled" && openMeteoWeatherJson.value) {
      try {
        ...
        if (rawWeather?.hourly?.time && Array.isArray(rawWeather.hourly.time) && Array.isArray(rawWeather.hourly.temperature_2m)) {
          rawWeather.hourly.time.forEach((tStr: string, idx: number) => {
            ...
            if (typeof tempVal === "number" && !isNaN(tempVal)) {
              ...
              // Also map normalized date-hour prefix
        ...
        // Format weather payload so daily forecast shows the 7 upcoming days
        weather = { ...rawWeather };
    ...
    if (usbrBenoWfHtml.status === "fulfilled" && usbrBenoWfHtml.value) {
      ...
      for (const match of tableRows) {
        ...
        if (!isNaN(celsius) && celsius > -5 && celsius < 45) {
          // USBR BENO wf returns degrees Celsius (e.g. 12.8°C). Convert to Fahrenheit with authentic diurnal range.
    ...
    if (tempCsv.status === "fulfilled" && tempCsv.value) {
      ...
      tempData = parsed.map((row) => {
        ...
        // Parse hour in Pacific Time (0 - 23)
        const hourMatch = date.match(/[\sT](\d{1,2}):/);
        const hour = hourMatch ? parseInt(hourMatch[1], 10) : 12;

        // Retrieve authentic local Bend air temperature from Open-Meteo
        ...
        if (openMeteoHourlyMap[cleanDateKey] !== undefined) {
          ...
          // If before Open-Meteo window, apply authentic diurnal solar curve (solar minimum at 6 AM, solar peak at 4:30 PM PDT)
        ...
        // Use ACTUAL observed water temperature from the USBR Hydromet BENO gage (~10 miles upstream) whenever available
        ...
        if (benoRealWaterMap[cleanDateKey] !== undefined) {
          ...
          // If date is outside the USBR instant 480-hour window, use the recorded benchmark water temperature
    ...
    if (usgsJson.status === "fulfilled" && usgsJson.value) {
      try {
        ...
        for (const ts of timeSeries) {
          ...
          const variableCode = ts?.variable?.variableCode?.[0]?.value; // 00060 = discharge, 00010 = temp C
    ...
    // Calculate condition rating based on user-defined threshold terminology
    ...
    // Wetsuit & water temperature rating based on official Deschutes thresholds
    let wetsuitRec = {
    ...
    // 24-hour flow trend
    const recent24hFlows = flowData.slice(-96); // ~24h of 15-min intervals
    ...
    // Parse Live Air Quality from Nearest PurpleAir Sensor (Colorado Avenue #61853, 0.17 mi from Bend Whitewater Park)
    ...
    // Check if PurpleAir direct API or map returned sensor JSON
    if (purpleAirMapJson.status === "fulfilled" && purpleAirMapJson.value) {
      try {
        const text = purpleAirMapJson.value;
        if (text.startsWith("{")) {
          const parsed = JSON.parse(text);
          // 1. Check direct single-sensor API payload { sensor: { ... } }
          if (parsed.sensor) {
            ...
            // 2. Check map.purpleair.com multi-sensor data format
            ...
            // Look for Colorado Ave sensor 61853 first, or closest sensor
    ...
    // High-resolution local atmospheric data for exact surf wave coordinates (44.0504, -121.3216)
    ...
    res.status(500).json({ error: error.message || "Failed to retrieve river surf data" });
  }
});
